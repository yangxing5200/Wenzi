...
namespace Maticsoft.Common
{
    ...
    public class DataToExcel
    {
        ...
        #region 通过路劲和EXCEL的后缀名来加载EXCEL到DATASET
        /// <summary>
        /// 通过路劲和EXCEL的后缀名来加载EXCEL到DATASET
        /// </summary>
        /// <param name="filePath">文件路径</param>
        /// <returns>返回表的集合</returns>
        public DataSet LoadDataFromExcel(string filePath, string isxls)
        {
           return  LoadDataFromExcel(filePath, isxls, "Sheet1");
        }

        public DataSet LoadDataFromExcel(string filePath, string isxls ,string fileName)
        {
            try
            {
                ...
                String sql =string.Format( "SELECT * FROM  [{0}$]",fileName);//可是更改Sheet名称，比如sheet2，等等 
